# AI 협업 가이드 (Codex / Cursor / 기타)

> 이 문서는 AI 도구가 이 프로젝트를 작업할 때 참고하는 가이드입니다.
> Codex는 자동으로 이 파일을 읽습니다. Cursor 등 다른 도구는 "AGENTS.md를 먼저 읽어줘" 라고 한 줄 알려주세요.
...
| 용어 | 의미 | 예시 |
|------|------|------|
| **단원** (unit) | 교과서의 큰 묶음 | "1단원 자연수의 혼합계산" |
...
### 중복 정보 금지 (한 번 보이면 충분한 정보는 한 번만)

학습지 출력에서 **상위 컨텍스트가 이미 알려주는 정보를 하위에서 반복하지 마라**. 시트 제목 = 한 번, 컨트롤바 = 드롭다운으로 충분, 개별 문제 = 그 자체로 자명한 도형/식만.

#### 위반 사례 (2026-05-02 일괄 정리 — 모두 제거됨)
| 위반 위치 | 중복된 상위 정보 |
|---|---|
| `divMethod` 개별 문제의 `${a}과 ${b}의 최대공약수 구하기` 라벨 | 시트 제목에 이미 "최대공약수" |
| 컨트롤바 메타 칩 `selectedSection` (`2단원 · 본단원`) | 단원 드롭다운 + 학습지 라벨 [본단원] 에 이미 있음 |
| 컨트롤바 메타 칩 `selectedLesson` (`교과 차시: ...`) | 학습지 선택만으로 충분 |
| 시트 제목 부제 `sheet-title-sub` (lessonRef) | 시트 메인 제목에 이미 단원/구분 정보 |
| u6 도형 학습지 개별 문제 prompt (`삼각형의 둘레를 구해 보세요.` 등) | 시트 제목과 SVG 그림이 자명함 |
| u6 직사각형 넓이 힌트 (`넓이 = 가로 × 세로`) | 학습지 전체에 동일하게 적용되는 공식. 매 문제 반복 X |
| u3 대응 관계 설명문 (`수가 늘어나면 ...`) | 학습 의도 없이 매 문제 반복되는 보조문 |

#### 새 학습지 추가 시 체크리스트
- [ ] 시트 제목이 알려주는 정보를 개별 문제에서 다시 쓰지 않았는가?
- [ ] 모든 문제에 동일한 prompt/힌트/note 가 들어가지 않았는가? (매 문제 반복은 의미 없음)
- [ ] 컨트롤바에 메타 칩/배지 추가하지 않았는가? (드롭다운 라벨로 충분)
- [ ] 학년·학기 출처 태그가 본단원에 붙지 않았는가? (선수학습만)
- [ ] 도형 SVG 자체가 문제를 설명해 주는데 굳이 prompt 를 추가하지 않았는가?

`shapeProblem(svg, prompt, answer, note)` 의 `prompt` 와 `note` 는 **빈 문자열** 로 두는 것을 기본값으로. 정말로 그림만으로는 의도가 안 전달될 때만 사용.
