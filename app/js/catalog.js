 ...
 * 카탈로그 빌더 시스템
 *
 * 기존: 학습지마다 9개 필드를 수동 입력
 * 개선: 단원/차시 단위로 묶고, 공통 속성은 상속
 *
 * 사용법:
 *   defineUnit('u1', '자연수의 혼합 계산', [
 *     선수학습({ grid: 'standard', count: 20 }, [
 *       학습지('두 자리 수 덧셈', genFn),
 *       학습지('두 자리 수 뺄셈', genFn, { count: 15 }),
 *     ]),
 *     차시('1차시 덧셈과 뺄셈', [
 *       학습지('혼합(순서)', genFn),
 *     ]),
...
import { T, bindTemplate } from './templates.js';

/* ── 빌더 함수들 ── */

let _idCounter = 0;
function autoId(unitId, label) {
  _idCounter++;
  const slug = label.replace(/[^가-힣a-zA-Z0-9]/g, '_').replace(/_+/g, '_').slice(0, 30);
  return `${unitId}_${_idCounter}_${slug}`;
}

/**
 * 선수학습 그룹
 * @param {Object} defaults - 기본 속성 { grid, count, kind }
 * @param {Array} items - 학습지 배열
 */
...
/**
 * 차시(본단원) 그룹
 * @param {string} lessonRef - 차시 설명
 * @param  {...any} args - [defaults], items
 */
 ...
 * @param {Function|Object} templateOrGen - 템플릿 객체 또는 제너레이터 함수
 * @param {Function} [genOrOverrides] - 제너레이터 함수 또는 오버라이드
 * @param {Object} [overrides] - 추가 오버라이드 { grid, count, ... }
 */
export function 학습지(label, templateOrGen, genOrOverrides, overrides) {
  // 학습지('라벨', generator)
  // 학습지('라벨', generator, { count: 15 })
  // 학습지('라벨', template, generator)
  // 학습지('라벨', template, generator, { count: 15 })
  let template = null;
  let generator;
  let opts = {};
...
/**
 * 단원 정의 → WORKSHEET_CATALOG 항목들로 변환
 */
export function defineUnit(unitId, unitName, groups) {
  const entries = [];
  ...
  for (const group of groups) {
    ...
    for (const item of items) {
      ...
      // 우선순위: overrides > defaults > templateDefaults > fallback
      ...
      const id = overrides.id || autoId(unitId, label);
...
export const PDF_WORKSHEET_MAP = {
  u1_pre_add_2d: 'add_2d',
  u1_pre_add: 'add_3d',
  u1_pre_sub_2d: 'sub_2d',
  u1_pre_sub: 'sub_3d',
  u1_pre_mul_2d1d: 'mul_2d1d',
  u1_pre_mul: 'mul_2d2d',
  u1_pre_mul_3x2: 'mul_3d2d',
  u1_pre_div_2d1d: 'div_2d1d',
  u1_pre_div_2d1d_norem: 'div_2d1d_norem',
  u1_pre_div_2d1d_rem: 'div_2d1d_rem',
  u1_pre_div_3d1: 'div_3d1d',
  u1_pre_div_3d2: 'div_3d2d',
};

/* ── 제너레이터 import & 카탈로그 구성 ── */
...
} from './generators/u6.js';

/* ──────────────────────────────────────────
   카탈로그 정의: 빌더 방식
   ────────────────────────────────────────── */

const u1 = defineUnit('u1', '자연수의 혼합 계산', [
  ...
  차시('1차시 덧셈과 뺄셈이 섞여 있는 식 계산하기', { grid: 'standard', count: 20 }, [
    학습지('덧셈·뺄셈 혼합(순서)', genU1MainAddSubOrder, { id: 'u1_main_addsub_order' }),
    학습지('덧셈·뺄셈 혼합(괄호)', genU1MainAddSubParen, { id: 'u1_main_addsub_paren' }),
    학습지('덧셈·뺄셈 혼합(괄호, 순서)', genU1MainAddSubParenOrder, { id: 'u1_main_addsub_mix' }),
  ]),
  차시('2차시 곱셈과 나눗셈이 섞여 있는 식 계산하기', { grid: 'standard', count: 20 }, [
    학습지('곱셈·나눗셈 혼합(순서)', genU1MainMulDivOrder, { id: 'u1_main_muldiv_order' }),
    학습지('곱셈·나눗셈 혼합(괄호)', genU1MainMulDivParen, { id: 'u1_main_muldiv_paren' }),
    학습지('곱셈·나눗셈 혼합(괄호, 순서)', genU1MainMulDivMix, { id: 'u1_main_muldiv' }),
  ]),
  차시('3차시 덧셈, 뺄셈, 곱셈이 섞여 있는 식 계산하기', { grid: 'standard', count: 20 }, [
    학습지('덧셈·뺄셈·곱셈 혼합(순서)', genU1MainAddSubMulOrder, { id: 'u1_main_addsubmul_order' }),
    학습지('덧셈·뺄셈·곱셈 혼합(괄호)', genU1MainAddSubMulParen, { id: 'u1_main_addsubmul_paren' }),
    학습지('덧셈·뺄셈·곱셈 혼합(괄호, 순서)', genU1MainAddSubMul, { id: 'u1_main_addsubmul' }),
  ]),
  차시('4차시 덧셈, 뺄셈, 나눗셈이 섞여 있는 식 계산하기', { grid: 'standard', count: 20 }, [
    학습지('덧셈·뺄셈·나눗셈 혼합(순서)', genU1MainAddSubDivOrder, { id: 'u1_main_addsubdiv_order' }),
    학습지('덧셈·뺄셈·나눗셈 혼합(괄호)', genU1MainAddSubDivParen, { id: 'u1_main_addsubdiv_paren' }),
    학습지('덧셈·뺄셈·나눗셈 혼합(괄호, 순서)', genU1MainAddSubDiv, { id: 'u1_main_addsubdiv' }),
  ]),
  차시('5차시 덧셈, 뺄셈, 곱셈, 나눗셈이 섞여 있는 식 계산하기', { grid: 'standard', count: 20 }, [
    학습지('사칙 혼합(순서)', genU1MainMixOrder, { id: 'u1_main_mix_order' }),
    학습지('사칙 혼합(괄호)', genU1MainMixParen, { id: 'u1_main_mix_paren' }),
    학습지('사칙 혼합(괄호, 순서)', genU1MainMix, { id: 'u1_main_mix' }),
  ]),
]);

const u2 = defineUnit('u2', '약수와 배수', [
  선수학습({ grid: 'dense', count: 36 }, [
    학습지('곱셈구구', genU2PreMul, { id: 'u2_pre_mul' }),
    학습지('나눗셈 (한 자리 나누기)', genU2PreDiv, { id: 'u2_pre_div' }),
  ]),
  차시('1차시 약수 이해하기', [
    학습지('약수 이해', genU2MainFactorConcept, { id: 'u2_main_factor', grid: 'concept', count: 10 }),
  ]),
  차시('2차시 배수 이해하기', [
    학습지('배수 이해', genU2MainMultipleConcept, { id: 'u2_main_multiple', grid: 'concept', count: 10 }),
  ]),
  차시('3차시 공약수와 최대공약수 이해하기', [
    학습지('공약수 이해', genU2MainCommonFactorConcept, { id: 'u2_main_common_factor', grid: 'concept', count: 10 }),
  ]),
  차시('4차시 두 수의 최대공약수 구하기', [
    학습지('최대공약수', T.divMethod, genU2MainGcdData, { id: 'u2_main_gcd', count: 6 }),
  ]),
  차시('5차시 공배수와 최소공배수 이해하기', [
    학습지('공배수 이해', genU2MainCommonMultipleConcept, { id: 'u2_main_common_multiple', grid: 'concept', count: 10 }),
  ]),
  차시('6차시 두 수의 최소공배수 구하기', [
    학습지('최소공배수', T.divMethod, genU2MainLcmData, { id: 'u2_main_lcm', count: 6 }),
  ]),
]);

const u3 = defineUnit('u3', '대응 관계', [
...
const u4 = defineUnit('u4', '약분과 통분', [
  ...
  차시('1차시 크기가 같은 분수 알아보기', { grid: 'practice', count: 15 }, [
    학습지('크기가 같은 분수 찾기', genU4MainEquivFind, { id: 'u4_main_equiv_find' }),
  ]),
  차시('2차시 크기가 같은 분수를 만드는 방법 알아보기', { grid: 'practice', count: 15 }, [
    학습지('크기가 같은 분수 만들기', genU4MainEquivMake, { id: 'u4_main_equiv_make' }),
  ]),
  차시('3차시 약분 알아보기', [
    학습지('약분 (과정 연습)', genU4MainRedProcess, { id: 'u4_main_red_process', grid: 'concept', count: 8 }),
    학습지('약분', genU4MainRed, { id: 'u4_main_red', grid: 'practice', count: 15 }),
  ]),
  차시('4차시 통분 알아보기', [
    학습지('통분 (과정 연습)', genU4MainLcdProcess, { id: 'u4_main_lcd_process', grid: 'concept', count: 8 }),
    학습지('통분', genU4MainLcd, { id: 'u4_main_lcd', grid: 'practice', count: 15 }),
  ]),
  차시('5차시 분수의 크기 비교하기', [
    학습지('분수 크기 비교', genU4MainCmp, { id: 'u4_main_cmp', grid: 'concept', count: 8 }),
  ]),
  차시('6차시 분수와 소수의 크기 비교하기', [
    학습지('분수와 소수 크기 비교', genU4MainDecCmp, { id: 'u4_main_dec_cmp', grid: 'concept', count: 8 }),
  ]),
]);

const u5 = defineUnit('u5', '분수의 덧셈과 뺄셈', [
  선수학습({ grid: 'practice', count: 15 }, [
  ...
  차시('1~2차시 (진분수)+(진분수) 계산하기', [
    학습지('분모 다른 진분수의 덧셈 (합이 1 미만)', T.fracLcdStep, genFracAddLt1, { id: 'u5_main_add' }),
    학습지('분모 다른 진분수의 덧셈 (합이 1 이상)', T.fracLcdStep, genFracAddGe1, { id: 'u5_main_add_ge1' }),
  ]),
  차시('3차시 (대분수)+(대분수) 계산하기', [
    학습지('대분수 덧셈 (가분수 변환)', T.mixedImproperStep, genMixedAddNoCarry, { id: 'u5_main_mixed_add_improper' }),
    학습지('대분수 덧셈 (자연수·분수 따로)', T.mixedSeparateStep, genMixedAddNoCarry, { id: 'u5_main_mixed_add_separate' }),
    학습지('받아올림 대분수 덧셈 (가분수 변환)', T.mixedImproperStep, genMixedAddCarry, { id: 'u5_main_mixed_add_carry_improper' }),
    학습지('받아올림 대분수 덧셈 (자연수·분수 따로)', T.mixedSeparateStep, genMixedAddCarry, { id: 'u5_main_mixed_add_carry_separate' }),
  ]),
  차시('4차시 (진분수)-(진분수) 계산하기', [
    학습지('분모 다른 진분수의 뺄셈', T.fracLcdStep, genFracSub, { id: 'u5_main_sub' }),
  ]),
  차시('5차시 (대분수)-(대분수) 계산하기(1)', [
    학습지('대분수 뺄셈 - 받아내림 없음 (가분수 변환)', T.mixedImproperStep, genMixedSubNoBorrow, { id: 'u5_main_mixed_sub_no_borrow_improper' }),
    학습지('대분수 뺄셈 - 받아내림 없음 (자연수·분수 따로)', T.mixedSeparateStep, genMixedSubNoBorrow, { id: 'u5_main_mixed_sub_no_borrow_separate' }),
  ]),
  차시('6차시 (대분수)-(대분수) 계산하기(2)', [
    학습지('대분수 뺄셈 - 받아내림 있음 (가분수 변환)', T.mixedImproperStep, genMixedSubBorrow, { id: 'u5_main_mixed_sub_borrow_improper' }),
    학습지('대분수 뺄셈 - 받아내림 있음 (자연수·분수 따로)', T.mixedSeparateStep, genMixedSubBorrow, { id: 'u5_main_mixed_sub_borrow_separate' }),
  ]),
]);

const u6 = defineUnit('u6', '다각형의 둘레와 넓이', [
  선수학습({ grid: 'standard', count: 20 }, [
    학습지('단위 변환 (cm, m, km)', genU6PreUnit, { id: 'u6_pre_unit' }),
    학습지('길이의 합과 차', genU6PreLen, { id: 'u6_pre_len' }),
  ]),
  차시('1차시 다각형의 둘레 구하기', [
    학습지('다각형의 둘레', genU6MainPerimeter, { id: 'u6_main_perimeter', grid: 'wide', count: 8 }),
  ]),
  차시('2차시 1㎠ 알아보기', [
    학습지('1㎠ 이해', genU6MainUnitSquare, { id: 'u6_main_unit_square', grid: 'wide', count: 8 }),
  ]),
  차시('3차시 직사각형의 넓이 구하기', [
    학습지('직사각형 넓이', genU6MainRect, { id: 'u6_main_rect', grid: 'wide', count: 8 }),
  ]),
  차시('4차시 1㎠보다 더 큰 넓이의 단위 알아보기', [
    학습지('넓이 단위 확장', genU6MainAreaUnit, { id: 'u6_main_area_unit', grid: 'standard', count: 20 }),
  ]),
  차시('5차시 평행사변형의 넓이 구하기', [
    학습지('평행사변형 넓이', genU6MainPara, { id: 'u6_main_para', grid: 'wide', count: 8 }),
  ]),
  차시('6~7차시 삼각형의 넓이 구하기', [
    학습지('삼각형 넓이', genU6MainTri, { id: 'u6_main_tri', grid: 'wide', count: 8 }),
  ]),
  차시('8~9차시 사다리꼴의 넓이 구하기', [
    학습지('사다리꼴 넓이', genU6MainTrap, { id: 'u6_main_trap', grid: 'wide', count: 8 }),
  ]),
  차시('10~11차시 마름모의 넓이 구하기', [
    학습지('마름모 넓이', genU6MainRhombus, { id: 'u6_main_rhombus', grid: 'wide', count: 8 }),
  ]),
  차시('3~11차시 도형 넓이 혼합', [
    학습지('도형 넓이 혼합', genU6MainMix, { id: 'u6_main_mix', grid: 'wide', count: 8 }),
  ]),
]);

/* ── 최종 카탈로그 ── */

export const WORKSHEET_CATALOG = [...u1, ...u2, ...u3, ...u4, ...u5, ...u6];
export const catalogMap = Object.fromEntries(WORKSHEET_CATALOG.map((item) => [item.id, item]));

export const PDF_GENERATORS = { ...PDF_GENERATORS_U1 };

/* ── 헬퍼 함수 ── */

export function getWorksheetsByUnit(unitId) {
  return WORKSHEET_CATALOG.filter((item) => item.unit === unitId);
}

export function buildSheetTitle(item) {
  const meta = UNIT_META[item.unit];
  return {
    main: `${meta.short} [${item.section}] ${item.label}`,
    sub: item.lessonRef,
  };
}
